

import 'package:client/controller/auth_provider.dart';
import 'package:client/model/job.dart';
import 'package:client/model/member.dart';
import 'package:client/model/static.dart';
import 'package:client/pages/arguments/static_arguments.dart';
import 'package:client/pages/dialog/static_dialog.dart';
import 'package:client/pages/static_overview.dart';
import 'package:client/service/http_service.dart';
import 'package:client/utils/responsive.dart';
import 'package:flutter/foundation.dart';
import 'package:flutter/material.dart';
import 'package:provider/src/provider.dart';

import '../constants.dart';

class StaticScreen extends StatefulWidget {

  static const String route = "statics";

  @override
  State<StaticScreen> createState() => _StaticScreenState();
}

class _StaticScreenState extends State<StaticScreen> {



  @override
  Widget build(BuildContext context) {


    return Column(
     // mainAxisSize: MainAxisSize.min,
      children: [
        //TODO SearchBar
        Row(
          children: [
            //Search // add
            SizedBox(
                width: Responsive.isMobile(context) ? 300 : 400,
                child: const TextField(
                  decoration: InputDecoration(
                    prefixIcon: Icon(Icons.search)
                  ),
                )
            ),
            const SizedBox(width: 20,),
            SizedBox(
              height: 50,
              child: Padding(
                padding: const EdgeInsets.only(left: 20, right:20),
                child: ElevatedButton(
                    onPressed: _addStatic,
                    child:  Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                        children: const [
                          Icon(Icons.add),
                          Text("add")
                        ],
                    )
                ),
              ),
            )
            //add
          ],
        ),
       const  SizedBox(height: 40,),
        FutureBuilder(
              future: HttpService().getStatics(),
                builder: (context, AsyncSnapshot<List<Static>> snapshot) {
                  List<Static> statics = snapshot.data ?? List.empty();
                 // statics = [...newStatics,...statics];

                  return  Flexible(
                    fit: FlexFit.loose,
                      child: SingleChildScrollView(
                        scrollDirection: Axis.vertical,
                        child: Wrap(
                          direction: Axis.horizontal,
                          children: statics.map((el) => StaticCard(static: el)).toList()
                        ),
                      ),
                  );
                }
        )
       // ListView.builder(itemBuilder: itemBuilder)
      ],
    );
  }

  _reload() {

    //rerender this shit

    setState(() {});
  }

  _addStatic() {

  showDialog(
      context: context,
      builder: (context) {
        return StaticDialog();
      },
  ).then((val) {
    _reload(); //TODO kinda not working WHAAAA
  });

  }

}


class StaticCard extends StatelessWidget {

  final Static static;

  StaticCard({required this.static});

  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () => _openStatic(context, static.id),
      child: SizedBox(
        width: Responsive.isMobile(context) ?  500 : 700,
        //height: 600,
        child: Card(
            color: cardColor,
          child:  Column(
              mainAxisAlignment: MainAxisAlignment.start,
              mainAxisSize: MainAxisSize.min,
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.start,
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                     Column(
                        children: [
                          SizedBox(
                            height: 300,
                            width: Responsive.isMobile(context) ? 200 : 400,
                            child:   static.staticImageUrl != "" ?
                            Image.network(static.staticImageUrl!, fit: BoxFit.cover,):
                            Container(color: Colors.black),
                          ),
                          SizedBox(height: 5,),
                          Text(static.name, style: Theme.of(context).textTheme.headlineMedium,),
                          SizedBox(height: 10,)
                        ],
                      ),
                    SizedBox(width: 20,),
                    Expanded(child: Container(),),
                    SizedBox(
                      width: 250,
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.start,
                        children: [
                          SizedBox(height: 25,),
                            Row(
                              children: const [
                                Icon(Icons.access_time, color: Colors.orange,),
                                SizedBox(width: 5,),
                                Text("Raid times"),
                             ],
                            ),
                          SizedBox(height: 15,),
                          //TODO
                          SizedBox(
                            height: 100,
                            child: ListView.builder(
                               itemCount:  static.schedules?.length ?? 0,
                                itemBuilder: (context, index) {
                                  return    Row(
                                    children:  [
                                      Text(static.schedules![index].day),
                                      SizedBox(width: 10,),
                                      Text(static.schedules![index].start.format(context)),
                                      SizedBox(width: 5,),
                                      Text("-"),
                                      SizedBox(width: 5,),
                                      Text(static.schedules![index].end.format(context))
                                    ],
                                  );
                                }
                            ),
                          ),

                          SizedBox(height: 5),
                          Divider(),
                          Row(
                            children: const [
                              Icon(Icons.people, color: Colors.orange,),
                              SizedBox(width: 5,),
                              Text("Members"),
                            ],
                          ),
                          SizedBox(height: 15,),
                          ClassList(members: static.members!),
                          SizedBox(height: 5),
                          Divider(),
                          Row(
                            children: const [
                              Icon(Icons.directions_run, color: Colors.orange,),
                              SizedBox(width: 5,),
                              Text("Progress"),
                            ],
                          ),
                          SizedBox(height: 15,),


                            //TODO progess

                            //footer with classes as icnons
                        ],
                      ),
                    )
                  ],
                ),
              ],
            ),

        ),
      ),
    );
  }

  _openStatic(BuildContext context, int? id) {
    //context.read<CustomMenuController>().pushNamed(HomeScreen.route);
    if(id != null) {
      Navigator.of(context).pushNamed(
          StaticOverview.route,
          arguments:  StaticArguments(
              id
          )
      );
    } else {
      //TODO
    }

  }

}

class ClassList extends StatelessWidget {

  List<Member> members;

  ClassList({
    required this.members
  });

  @override
  Widget build(BuildContext context) {

    List<Widget> jobIcons = [];
    for(Member m in members) {
        jobIcons.add(JobIcon(job: m.job!, active: true,));
        //m.job == "WAR", "PLD", "DRK" // etc."
    }

    return  Wrap(
        alignment: WrapAlignment.start,
        children: jobIcons
    );
  }
}

class JobIcon extends StatelessWidget {

  String job;
  bool active;
  double? size;

  JobIcon({
    required this.job,
    required this.active,
    this.size = 22,
  });

  @override
  Widget build(BuildContext context) {
    return SizedBox(
     height: size,
     width: size,
      child:  Opacity(
          opacity: active ? 1 : 0.5,
          child: Image.asset(_getIcon()),
      ),
    );
  }

  String _getIcon() {
    if(job == "dps") {
      return "assets/images/role/DPSRole.png";
    } else if(job == "heal") {
      return "assets/images/role/HealerRole.png";
    } else if(job == "tank") {
      return "assets/images/role/TankRole.png";
    } else {
      return "assets/images/jobs/"+job.toString()+".png";
    }
  }
}
